...
class EmailNotifier:
    ...
    def should_notify(site: Site, new_status: StatusType, old_status: StatusType) -> bool:
        ...
        if old_status == StatusType.OPERATIONAL and new_status != StatusType.OPERATIONAL:
            return True

        # Notify on recovery to operational (after being non-operational)
        if old_status != StatusType.OPERATIONAL and new_status == StatusType.OPERATIONAL:
            # Only if we previously notified about the degradation
            if site.last_notified_status and site.last_notified_status != StatusType.OPERATIONAL:
                return True
